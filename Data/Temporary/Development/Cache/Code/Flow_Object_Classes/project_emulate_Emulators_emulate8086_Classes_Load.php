...
class Load extends Load_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		if ('project\emulate\Emulators\emulate8086\Classes\Load' === get_class($this)) {
	...
	$reflectedClass = new \ReflectionClass('project\emulate\Emulators\emulate8086\Classes\Load');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionService->isPropertyAnnotatedWith('project\emulate\Emulators\emulate8086\Classes\Load', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('project\emulate\Emulators\emulate8086\Classes\Load', $propertyName, 'var');
	 ...
	 */
	 private function Flow_Proxy_injectProperties() {
		$this->memory = new \project\emulate\Emulators\emulate8086\Classes\Domain\Model\Memory();
		$this->flags = new \project\emulate\Emulators\emulate8086\Classes\Domain\Model\Flags();
		$this->registers = new \project\emulate\Emulators\emulate8086\Classes\Domain\Model\Registers();
		$this->segments = new \project\emulate\Emulators\emulate8086\Classes\Domain\Model\Segments();
		$memoryRepository_reference = &$this->memoryRepository;
		$this->memoryRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getInstance('project\emulate\Emulators\emulate8086\Classes\Domain\Repository\MemoryRepository');
		if ($this->memoryRepository === NULL) {
			$this->memoryRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->getLazyDependencyByHash('92122aba64ddcfabe82346d69818bcd2', $memoryRepository_reference);
			if ($this->memoryRepository === NULL) {
				$this->memoryRepository = \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->createLazyDependency('92122aba64ddcfabe82346d69818bcd2',  $memoryRepository_reference, 'project\emulate\Emulators\emulate8086\Classes\Domain\Repository\MemoryRepository', function() { return \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->get('project\emulate\Emulators\emulate8086\Classes\Domain\Repository\MemoryRepository'); });
			}
		}
